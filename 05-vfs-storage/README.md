...
When a container starts in 200 milliseconds while the equivalent virtual machine takes 30 seconds, the difference is not CPU speed or network bandwidth. It is the filesystem. A VM must copy a disk image, boot an OS, and mount a filesystem before a single process can run. A container skips all of this: OverlayFS stacks the read-only image layers that already exist on the node and presents a merged view. Nothing is copied. The container's rootfs is assembled by the VFS in microseconds from layers that a hundred other containers may be sharing simultaneously.

This fast path only works because Linux has a carefully designed abstraction boundary between the VFS layer and the underlying filesystems. VFS defines four objects — `super_block` (mounted filesystem), `inode` (file or directory), `dentry` (name in a directory tree), and `file` (open file descriptor) — each backed by a vtable of function pointers. ext4 provides one implementation. tmpfs provides another. overlayfs provides a third that delegates through to other filesystems for the actual data. The kernel does not need to know which filesystem is underneath; it calls the vtable. This is why Kubernetes can mount a ConfigMap (backed by tmpfs), a PVC (backed by ext4 or Ceph), a Secret (backed by tmpfs), and a container image layer (backed by overlayfs) all inside the same container and have `open(2)` work identically on all of them.
